  ...
  return (
    <header 
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
        isScrolled || isMenuOpen ? 'bg-brand-surface shadow-md py-3' : 'bg-transparent py-4'
      }`}
    >
      <div className="max-w-5xl mx-auto px-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className={`text-xl font-bold tracking-tight ${isScrolled || isMenuOpen ? 'text-brand-text' : 'text-brand-text md:text-white'}`}>
            VOERAPIDO
          </span>
        </div>
        ...
        <button 
          ...
          aria-label="Menu"
        >
          {isMenuOpen ? <X size={24} /> : <Menu size={24} color={isScrolled ? '#2C3E50' : '#2C3E50'} />} 
          {/* Note: In a real 'hero image' scenario, the menu icon color might need adjustment based on background, but here we keep it safe with dark text or we can make it conditional */}
        </button>
          ...
          {navLinks.map((link) => (
            <a 
              ...
              className={`text-sm font-medium hover:text-brand-primary transition-colors ${
                isScrolled ? 'text-brand-text' : 'text-white drop-shadow-md'
              }`}
            >
              {link.name}
            </a>
